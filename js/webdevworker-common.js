...
(function () {
      ...
      function bulletproofCopy(text, target, btn, okMsg) {
        ...
        function triggerSuccess(customMsg) {
          ...
          if (btn) {
            ...
            // NOTE: exclude .ww-ripple — the pointerdown ripple appends a decorative
            // span to the button; it must never be mistaken for the label span.
            var spans = btn.querySelectorAll('span:not(.ww-ripple)');
            if (spans.length >= 2) {
              spans[0].textContent = '✓';
  ...
  // 5c. AI Studio nav group — injected site-wide so every page's sidebar lists the 11 instruments.
  function injectAIStudioNav() {
    try {
      var nav = document.querySelector('.sidebar-nav');
      if (!nav || document.getElementById('wwAiStudioNav')) return;
      var tools = [
        ['ai-micro-app-smith', '🏭', 'AI Micro-App Smith'],
        ['ai-regex-smith', '🔣', 'AI Regex Smith'],
        ['ai-code-doctor', '🩺', 'AI Code Doctor'],
        ['ai-code-reviewer', '🔍', 'AI Code Reviewer'],
        ['ai-security-auditor', '🛡️', 'AI Security Auditor'],
        ['ai-sql-smith', '🗄️', 'AI SQL Smith'],
        ['ai-test-forge', '🧪', 'AI Test Forge'],
        ['ai-api-oracle', '📡', 'AI API Oracle'],
        ['ai-prompt-surgeon', '✂️', 'AI Prompt Surgeon'],
        ['ai-refactor', '🔄', 'AI Refactor'],
        ['ai-code-explainer', '💡', 'AI Code Explainer']
      ];
      var items = tools.map(function (t) {
        return '<li><a href="/tools/' + t[0] + '.html" class="nav-item sidebar-link" title="' + t[2] + '" style="display:flex;align-items:center;gap:10px;padding:8px 12px;text-decoration:none;color:var(--text-main);font-size:0.84rem;border-radius:var(--radius-sm);margin:1px 4px;font-weight:600;transition:all 0.15s ease;">' +
          '<span style="font-size:1.05rem;flex-shrink:0;width:20px;text-align:center;">' + t[1] + '</span>' +
          '<span class="nav-item-title" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:-0.01em;">' + t[2] + '</span></a></li>';
      }).join('');
      nav.insertAdjacentHTML('afterbegin',
        '<div class="nav-group-wrapper" id="wwAiStudioNav" style="margin-bottom:14px;">' +
        '<div class="nav-group-title" style="font-size:0.75rem;text-transform:uppercase;color:#10b981;background:rgba(16,185,129,0.08);border-left:3px solid #10b981;padding:6px 12px;margin:12px 6px 6px;border-radius:0 6px 6px 0;font-weight:800;letter-spacing:1px;">✦ AI Studio</div>' +
  ...
  function runAll() {
    ...
    var inits = [initClipboardHardening, syncTheme, injectGuides, injectArticleBack,
                 initSidebar, initMobileDrawer, initSearch, initCategoryFilter, injectAIStudioNav, initGlobalSearchDropdown,
